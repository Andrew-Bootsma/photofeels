This is a photo searching and photographer info site created for my friends at Fullscript.

## To Build

1. Fork the repository.

2. In the project directory, you can run:

### `npm i`

to install your copy of `/node_modules`.

3. You will need to create a:

### `.env.local` 

file in the root of the application. Add a `REACT_APP_UNSPLASH_CLIENT_ID='YOUR-ID-GOES-HERE'` variable to the file. Replace `YOUR-ID-GOES-HERE` with an Unsplash client id generated by creating an account here --> https://unsplash.com/developers.

4. Finally, you can run:

### `npm start`

Runs the app in the development mode.<br>
Open [http://localhost:3000](http://localhost:3000) to view it in the browser.

The page will reload if you make edits.<br>

## Photo Feels!
![alt text](https://andrewbootsma.com/wp-content/uploads/2019-08-28-133831_1366x768_scrot.png)

### What I am proud of
- The react is very meticulously done and is where I spent the bulk of my time
- I implemented a few different features of the api to add to the experience of the app
- I couldn't resist a few minutes of graphic design to create my own logo

### If I had more time
- I would finish all of the testing, I wanted to focus on what I am best at first which is why I left testing for the end
  - I managed to install Jest and Enzyme and begin setting up my tests but I could not fully test my features
  - I am still learning how to TDD and BDD from the start
- I am happy with about 90% of my SASS but I could clean up a few sections, such as the photographer profile page

### Production
- https://photo-feels-by-andrew-bootsma.netlify.com/

### Stack Description
- I used create-react-app to generate a boilerplate react app
- I installed SASS
  - I used BEM for my classNames
  - I used a simpler architecture than 7-1 because its a small application
  - It is fully responsive on all of the screens I tested
- I used the Context API instead of Redux because its a small application
